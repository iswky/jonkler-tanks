...
static void drawSpreadLines(SDL_Renderer* renderer, int x0, int y0, int x1, int y1, int offset) {
    for (int i = 0; i <= 1; i++) SDL_RenderDrawLine(renderer, x0, y0 + i, x1 + offset, y1 + offset + i);
    for (int i = 0; i <= 1; i++) SDL_RenderDrawLine(renderer, x0, y0 + i, x1 - offset, y1 - offset + i);
}
...
void renderSpreadArea(App* app, RenderObject* spreadArea) {
    ...
    if (app->currPlayer->tankGunObj->data.texture.flipFlag == SDL_FLIP_HORIZONTAL) {
        drawSpreadLines(
            app->renderer,
            relativePos.x + relativePos.x + 15 * currGunCos + spreadArea->data.texture.constRect.w,
            -relativePos.y + spreadArea->data.texture.constRect.h / 1.5 - 18 * currGunSin,
            relativePos.x + relativePos.x + 15 * currGunCos * 5 + spreadArea->data.texture.constRect.w,
            -relativePos.y + spreadArea->data.texture.constRect.h / 1.5 - 18 * currGunSin * 5,
            getMaxSpreadByWeapon(app->currWeapon)
        );
    } else {
        drawSpreadLines(
            app->renderer,
            relativePos.x + 20 * currGunCos,
            -relativePos.y + spreadArea->data.texture.constRect.h / 1.5 - 18 * currGunSin,
